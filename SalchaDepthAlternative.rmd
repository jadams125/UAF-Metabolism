...
#moved here because RMD is stupid
Salcha.exo.2021 <- read.csv(here("SALCHA.EXO.cl.corr.2021.csv"))
...
# for storms im ok with a 24 hour gap
...
CRREL_SALCHA <- merge(CRREL_SALCHA, ts_2021.salcha, by = "datetimeAK")

CRREL_SALCHA %>% ggplot(aes(x = as.POSIXct(datetimeAK), y = depth)) + geom_miss_point()
CRREL_SALCHA %>% ggplot(aes(x = as.POSIXct(datetimeAK), y = discharge)) + geom_miss_point()
CRREL_SALCHA %>% ggplot(aes(x = as.POSIXct(datetimeAK), y = DO.obs)) + geom_miss_point()
CRREL_SALCHA %>% ggplot(aes(x = as.POSIXct(datetimeAK), y = DO.sat)) + geom_miss_point()
CRREL_SALCHA %>% ggplot(aes(x = as.POSIXct(datetimeAK), y = temp.water)) + geom_miss_point()
CRREL_SALCHA %>% ggplot(aes(x = as.POSIXct(datetimeAK), y = ParMetCor)) + geom_miss_point()
...
write.csv(save, "C:/Users/jacob/OneDrive - University of Alaska/GitHub/UAF-Metabolism/outputs/salcha2021.comb.csv")
# write.csv(final.salcha.DT, here("UAF-Metabolism", "outputs", "correctedSalchaDT.csv"))


final.salcha.DT <- final.salcha.DT %>% 
  filter(as.character(solar.time) >= "2021-05-18 04:13:57") %>% filter(as.character(solar.time) <"2021-08-01 04:13:57")
...
bayes_specs <- specs(bayes_name, 
                     burnin_steps=9000, saved_steps=2000, n_cores=8, n_chains = 4, GPP_daily_lower = 0, ER_daily_upper = 0
)
...
save(mm.test.salcha, file = (here("Outputs","salcha.2019.1.RData")))

fit.salcha <- get_fit(mm.test.salcha)
fit.daily <- fit.salcha$daily
write.csv(fit.daily, file = (here("Outputs","salcha.2019.1.csv")))
...
k600mcmc <- rstan::traceplot(mcmc, pars='K600_daily', ncol=3)
gppmcmc <- rstan::traceplot(mcmc, pars='GPP_daily', ncol=3)
ermcmc <- rstan::traceplot(mcmc, pars='ER_daily', ncol=3)
...
final.salcha.DT <- read.csv(here("outputs", "salcha2021.comb.csv"))

final.salcha.DT$solar.time <- as.POSIXct(final.salcha.DT$solar.time, tz = "UTC")


final.salcha.DT <- final.salcha.DT %>% select(solar.time, DO.obs, DO.sat, temp.water, light, discharge, depth)

final.salcha.DT <- final.salcha.DT %>% 
  filter(as.character(solar.time) >= "2021-07-15 04:13:57") %>% filter(as.character(solar.time) < "2021-09-01 04:13:57")
...
bayes_specs <- specs(bayes_name, 
                     burnin_steps=10000, saved_steps=2000, n_cores=8, n_chains = 4, GPP_daily_lower = 0, ER_daily_upper = 0
)
...
difftime(endTime, startTime)
save(mm.test.salcha, file = (here("Outputs","salcha.2019.2.RData")))

fit.salcha <- get_fit(mm.test.salcha)
fit.daily <- fit.salcha$daily
write.csv(fit.daily, file = (here("Outputs","salcha.2019.2.csv")))
```
...
final.salcha.DT <- final.salcha.DT %>% 
  filter(as.character(solar.time) >= "2021-09-01 04:13:57") %>% filter(as.character(solar.time) <"2021-10-01 04:13:57")
...
difftime(endTime, startTime)
save(mm.test.salcha, file = (here("Outputs","salcha.2019.3.RData")))

fit.salcha <- get_fit(mm.test.salcha)
fit.daily <- fit.salcha$daily
write.csv(fit.daily, file = (here("Outputs","salcha.2019.3.csv")))
