...
data.salcha.mm.all <- data.salcha.mm.all %>% filter(as.character(solar.time) <= "2021-07-17 04:13:54")
...
data.salcha.mm.all <- data.salcha.mm.all %>%
  mutate(across(c(DO.obs),
                ~ifelse(as.character(solar.time) >= "2021-05-21 04:13:54" & as.character(solar.time) < "2021-05-21 04:13:54", NA, .)))
...
save(mm.test.salcha, file = here("Outputs", "salcha.2019.1.new.RData"))
# 
fit.salcha <- get_fit(mm.test.salcha)
fit.daily <- fit.salcha$daily
write.csv(fit.daily, here("outputs", "salcha.2019.1.new.full.csv"))
...
write.csv(model_data, here("outputs", "salcha.2019.1.new.model_data.csv"))
