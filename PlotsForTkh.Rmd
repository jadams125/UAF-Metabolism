...
#combine
metab_all <- rbind(strt.gpp, vaul.gpp, poke.gpp, frch.gpp, moos.gpp,craw.gpp.ts,mast.gpp.ts,shov.gpp.ts) 
...
#combine
metab_all <- rbind(strt.er, vaul.er, poke.er, frch.er, moos.er,craw.er.ts,mast.er.ts,shov.er.ts) 
metab_all$DOY <- yday(as.POSIXct(metab_all$date))
...
metab.frch2 <- metab_all_stw %>% filter(site == "FRCH")
metab.moos2 <- metab_all_stw %>% filter(site == "MOOS")
metab.poke2 <- metab_all_stw %>% filter(site == "POKE")
metab.vaul2 <- metab_all_stw %>% filter(site == "VAUL")
metab.strt2 <- metab_all_stw %>% filter(site == "STRT")
...
#combine
metab_all <- rbind(strt.nep, vaul.nep, poke.nep, frch.nep, moos.nep,craw.nep.ts,mast.nep.ts,shov.nep.ts) 
