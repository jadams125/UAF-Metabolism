...
Q_2022 <- read.csv("C:/Users/jacob/OneDrive - University of Alaska/GitHub/DoD_Discharge/Predicted_Discharge/2022/Predicted_Q_2022.csv")
Q_2022$DateTime <-  force_tz(as.POSIXct(ymd_hms(Q_2022$DateTime)), tz = "America/Anchorage")

#### Breaking up into sites ####
POKE_Q_2022 <- Q_2022 %>% select(DateTime, pred.poke.Q) %>% rename(Q = pred.poke.Q)
POKE_Q_2022$Site <- "POKE"
VAUL_Q_2022 <- Q_2022 %>% select(DateTime, pred.vaul.Q) %>% rename(Q = pred.vaul.Q)
VAUL_Q_2022$Site <- "VAUL"
STRT_Q_2022 <- Q_2022 %>% select(DateTime, pred.strt.Q) %>% rename(Q = pred.strt.Q)
STRT_Q_2022$Site <- "STRT"
MOOS_Q_2022 <- Q_2022 %>% select(DateTime, pred.moos.Q) %>% rename(Q = pred.moos.Q)
MOOS_Q_2022$Site <- "MOOS"
FRCH_Q_2022 <- Q_2022 %>% select(DateTime, pred.frch.Q) %>% rename(Q = pred.frch.Q)
...
MOOS.comb.in$datetimeAK <- as.POSIXct( MOOS.comb.in$datetimeAK)
...
MOOS.2019_comb_ts <- full_join(MOOS.comb.in, ts_2020.moos, by = "datetimeAK")
...
clean <- clean %>% mutate(across(c(DO.sat, DO.sat.EXO), ~ifelse(as.character(datetimeAK) >= "2019-06-13 13:45:00" & as.character(datetimeAK) <= "2019-06-13 16:00:00", NA, .)))
...
clean.frch.2020 <- clean %>% filter(as.character(solar.time) < "2020-09-03 04:13:57")
...
data.frch.mm.all <- rbind(clean.frch.2019,clean.frch.2020, clean.frch.2021)
...
data.vaul.mm.all <- rbind(clean.vaul.2019,clean.vaul.2020, clean.vaul.2021)
...
```{r}
data.vaul.mm.all <- rbind(CRREL_VAUL.2019, CRREL_VAUL.2020, CRREL_VAUL.2021)
...
data.strt.mm.all <- rbind(clean.strt.2019,clean.strt.2020, clean.strt.2021)
...
```{r}
data.strt.mm.all <- rbind(CRREL_STRT.2019, CRREL_STRT.2020, CRREL_STRT.2021)
...
data.poke.mm.all <- rbind(clean.poke.2019,clean.poke.2020, clean.poke.2021)
...
```{r}
data.poke.mm.all <- rbind(CRREL_POKE.2019, CRREL_POKE.2020, CRREL_POKE.2021)
